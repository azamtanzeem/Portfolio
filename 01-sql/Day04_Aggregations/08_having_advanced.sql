...
FROM (
    SELECT
        policy_holder_id,
        COUNT(case_id) AS call_count        -- count calls per holder
    FROM callers
    GROUP BY policy_holder_id
    HAVING COUNT(case_id) >= 3             -- only frequent callers
) AS call_records;                         -- subquery becomes a derived table
...
--
-- WHY a subquery here?
--   You can't do COUNT(HAVING ...) in one level.
